...
typedef void (*ble_lbs_data_handler_t) (ble_lbs_t * p_lbs, uint8_t * p_data, uint16_t length);
...
typedef struct
{
	...
	ble_lbs_data_handler_t  lbs_data_handler;
} ble_lbs_init_t;
...
struct ble_lbs_s
{
    ...
    ble_lbs_data_handler_t      lbs_data_handler;   /**< Event handler to be called when the LED Characteristic is written. */
};
...
uint32_t ble_lbs_string_send(ble_lbs_t * p_lbs, uint8_t * p_string, uint16_t length);
