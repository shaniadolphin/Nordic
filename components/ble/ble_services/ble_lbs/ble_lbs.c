...
static void on_write(ble_lbs_t * p_lbs, ble_evt_t * p_ble_evt)
{
    ...
    if ((p_evt_write->handle == p_lbs->led_char_handles.value_handle) &&
        (p_lbs->lbs_data_handler != NULL))
    {
        p_lbs->lbs_data_handler(p_lbs, p_evt_write->data, p_evt_write->len);
...
uint32_t ble_lbs_init(ble_lbs_t * p_lbs, const ble_lbs_init_t * p_lbs_init)
{
    ...
    p_lbs->conn_handle       = BLE_CONN_HANDLE_INVALID;
    p_lbs->lbs_data_handler = p_lbs_init->lbs_data_handler;
...
uint32_t ble_lbs_on_button_change(ble_lbs_t * p_lbs, uint8_t button_state)
{
    ...
    params.p_data = &button_state;//buf;//
    params.p_len = &len;
