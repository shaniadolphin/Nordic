...
static void on_hvx(ble_lbs_c_t * p_ble_lbs_c, const ble_evt_t * p_ble_evt)
{
    ...
    if (p_ble_evt->evt.gattc_evt.params.hvx.handle == p_ble_lbs_c->peer_lbs_db.button_handle)
    {
        if (p_ble_evt->evt.gattc_evt.params.hvx.len != 0)
        {
			...
			ble_lbs_c_evt.data_len = p_ble_evt->evt.gattc_evt.params.hvx.len;	
			//ble_lbs_c_evt.rssi = adv_report.rssi;
			sd_ble_gap_rssi_get(p_ble_lbs_c->conn_handle, &rssi);
			ble_lbs_c_evt.rssi = rssi;
            ...
            ble_lbs_c_evt.conn_handle                = p_ble_lbs_c->conn_handle;
            ble_lbs_c_evt.params.button.button_state = p_ble_evt->evt.gattc_evt.params.hvx.data[0];
			//ble_lbs_c_evt.params.p_data   = (uint8_t *)p_ble_evt->evt.gattc_evt.params.hvx.data;
			//ble_lbs_c_evt.params.data_len = p_ble_evt->evt.gattc_evt.params.hvx.len;			
            p_ble_lbs_c->evt_handler(p_ble_lbs_c, &ble_lbs_c_evt);
#endif			
        }
    }
        ...
        p_evt->params.discovered_db.srv_uuid.type == p_ble_lbs_c->uuid_type)
    {
        ...
        for (i = 0; i < p_evt->params.discovered_db.char_count; i++)
        {
            ...
            switch (p_char->characteristic.uuid.uuid)
            {
                case LBS_UUID_LED_CHAR:
                    evt.peer_db.led_handle = p_char->characteristic.handle_value;
                    break;
                case LBS_UUID_BUTTON_CHAR:
                    evt.peer_db.button_handle      = p_char->characteristic.handle_value;
                    evt.peer_db.button_cccd_handle = p_char->cccd_handle;
                    break;
        ...
        if (p_ble_lbs_c->conn_handle != BLE_CONN_HANDLE_INVALID)
        {
            if ((p_ble_lbs_c->peer_lbs_db.led_handle         == BLE_GATT_HANDLE_INVALID)&&
                ...
                (p_ble_lbs_c->peer_lbs_db.button_cccd_handle == BLE_GATT_HANDLE_INVALID))
            {
                p_ble_lbs_c->peer_lbs_db = evt.peer_db;
            }
        }
...
uint32_t ble_lbs_mydata_send(ble_lbs_c_t * p_ble_lbs_c, uint8_t *databuf, uint8_t databuf_len)
{
    uint8_t i;
	VERIFY_PARAM_NOT_NULL(p_ble_lbs_c);
    ...
    //NRF_LOG_DEBUG("writing LED status 0x%x\r\n", status);
	NRF_LOG_INFO("LBS mydata send %d\r\n",p_ble_lbs_c->conn_handle);
    tx_message_t * p_msg;
    ...
    p_msg->req.write_req.gattc_params.handle   = p_ble_lbs_c->peer_lbs_db.led_handle;
    p_msg->req.write_req.gattc_params.len      = databuf_len;
    p_msg->req.write_req.gattc_params.p_value  = p_msg->req.write_req.gattc_value;
    p_msg->req.write_req.gattc_params.offset   = 0;
    p_msg->req.write_req.gattc_params.write_op = BLE_GATT_OP_WRITE_CMD;
    for(i = 0;i < databuf_len;i++)
		p_msg->req.write_req.gattc_value[i]    = databuf[i];
    p_msg->conn_handle                         = p_ble_lbs_c->conn_handle;
    p_msg->type                                = WRITE_REQ;	
    tx_buffer_process();
    return NRF_SUCCESS;
