...
static const char m_target_periph_name[] = "Nordic_Blinky";//"Nordic_Blinky";                  /**< Name of the device we try to connect to. This name is searched for in the scan report data*/
...
static void lbs_c_evt_handler(ble_lbs_c_t * p_lbs_c, ble_lbs_c_evt_t * p_lbs_c_evt)
{
	...
	switch (p_lbs_c_evt->evt_type)
    {
        ...
        {
            printk("Link 0x%x, Button state changed on peer to 0x%x\r\n",
                           p_lbs_c_evt->conn_handle,
			...
			LED_P20x40Char(54, 1, p_lbs_c_evt->params.button.button_state%10, 1);	
            err_code = led_status_send_to_all(p_lbs_c_evt->params.button.button_state);
            if (err_code == NRF_SUCCESS)
            {
                printk("LBS write LED state %d\r\n", p_lbs_c_evt->conn_handle);
            }			
        } 
...
static uint32_t led_status_send_to_all(uint8_t button_action)
{
    ...
    for (uint32_t i = 0; i< CENTRAL_LINK_COUNT; i++)
    {
        ...
        err_code = ble_lbs_mydata_send(&m_ble_lbs_c[i], &button_action, 1);
		if (err_code != NRF_SUCCESS &&
    ...
    switch (pin_no)
    {
        case 15:
			if(button_action   == 1)
			{
				...
				LED_P20x40Char(54, 1, 10, 1);	
				if (err_code == NRF_SUCCESS)
				{
					printk("LBS write LED state %d, %d\r\n", button_action, button_action);
				}
				else
				{
					printk("LBS write LED state error %d\r\n", button_action);
				}
			}
...
static void buttons_init(void)
{
   ...
   //The array must be static because a pointer to it will be saved in the button handler module.
    static app_button_cfg_t buttons[] =
    {
		...
		{14, false, BUTTON_PULL, button_event_handler},
        {13, false, BUTTON_PULL, button_event_handler},
		{12, false, BUTTON_PULL, button_event_handler}
		//{11, false, BUTTON_PULL, button2_event_handler}
    };
