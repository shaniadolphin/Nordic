...
#define DEVICE_NAME                     "Solar_Power"//"Solar_Power"                             /**< Name of device. Will be included in the advertising data. */

#define APP_ADV_INTERVAL                1500                                        /**< The advertising interval (in units of 0.625 ms; this value corresponds to 40 ms). */
...
#define VOLMEAS_READ_INTERVAL           APP_TIMER_TICKS(100, APP_TIMER_PRESCALER)  /**< ADC采样间隔 1000ms读取一次*/
...
static void leds_init(void)
{
	...
	nrf_gpio_pin_write(PIN_LED1,  1);//LEDS_ACTIVE_STATE ? 1 : 0);
...
static void button_event_handler(uint8_t pin_no, uint8_t button_action)
{
    ...
    switch (pin_no)
    {
        case PIN_KEY_IN:				
			if(button_action == 0)
			{
				...
				buf[0] = PIN_KEY_IN;		
				err_code = ble_nus_string_send(&m_lbs, buf, 5);
	...
	//printk("button_event_handler:%d-%d\r\n",pin_no,keycnt);		
}
...
static void volmeas_timeout_handler(void * p_context)
{
	...
	APP_ERROR_CHECK(err_code);
	NRF_LOG_RAW_INFO("volmeas_timeout_handler\r\n");
}
/**********************************************************************************************
 * 描  述 : ADC中断处理函数
 * 参   数: 无
...
void saadc_callback(nrf_drv_saadc_evt_t const * p_event)
{
	...
	if (p_event->type == NRF_DRV_SAADC_EVT_DONE)
    {
        ...
        APP_ERROR_CHECK(err_code);
		a  = 2.0 * 6.0 * ((float)(p_event->data.done.p_buffer[0])) * 600.0 / 1024.0;
		usb_vol  = 2.0 * 6.0 * ((float)(p_event->data.done.p_buffer[1])) * 600.0 / 1024.0;
		//#if defined(NRF_LOG_USES_RTT) && NRF_LOG_USES_RTT == 1
		//SEGGER_RTT_printf(0,"%d:saadc0=%d\r\n",NRF_RTC1->COUNTER,batt_vol);
		//SEGGER_RTT_printf(0,"%d:saadc1=%d\r\n",NRF_RTC1->COUNTER,usb_vol);
		//#endif		printk
        printk("saadc0=%4f\r\n", (float)batt_vol);//buf[0] for battery
		printk("saadc1=%4f\r\n", (float)usb_vol);//buf[1] for USB
    }
	...
	//#if defined(NRF_LOG_USES_RTT) && NRF_LOG_USES_RTT == 1
	//NRF_LOG_RAW_INFO(0,"myaddr:%d\r\n",myaddr[0]);
	//#endif
	//NRF_LOG_RAW_INFO("myaddr:%d\r\n",myaddr[0]);
...
void gpio_init(void)
{
	...
	nrf_gpio_pin_set(PIN_SOLAR_VOL2);	
